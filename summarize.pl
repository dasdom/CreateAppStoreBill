...
#	    print OUT "\\vspace{1cm}\n";
        ...
        #print "amount_key $common_key amount_value $amount_value\n";
	...
	$app_id{$common_key} = $line_components[10];
	$currency{$common_key} = $line_components[8];
	$price{$common_key} = $line_components[6];
    }
    close (FILE);
   
    my @keys = keys %amount;
    my @amount_values = values %amount;
    my @app_id_values = values %app_id;
    my @currency_values = values %currency;
    my @price_values = values %price;
    while (@keys) {
